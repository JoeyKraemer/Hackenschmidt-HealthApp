...
class CalorieCalculator {
    ...
    func calculateBaseMetabolicRate() -> Double {
        ...
        switch self.sex {
        case "Female":
            weightFactor = 9.247 * Double(self.weight!)
            heightFactor = 3.098 * Double(self.height!)
            ageFactor = 4.330 * Double(self.age!)
            baseValue = 447.593
        case "Male":
            weightFactor = 13.397 * Double(self.weight!)
            heightFactor = 4.799 * Double(self.height!)
            ageFactor = 5.677 * Double(self.age!)
            baseValue = 88.362
        default:
            return 0
        }
